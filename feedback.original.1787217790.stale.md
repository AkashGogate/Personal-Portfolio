...
**Why:** Squarespace repeatedly revert/lock apex A records back to own presets even after change to GitHub Pages IPs. Cause error recur across sessions.
...
**Why:** Stated apex A-record override "worked before, confirmed live" from compressed memory title alone. User push back ("are we sure about that?"). Full observation + live dig show it work once but since reverted — title alone misleading about current state.
...
**Why:** Plugins/MCP servers third-party installs — disposable, cheap re-enable. Self-authored personal skills weigh different even when usage data identical. Zero usage count not sufficient signal skill unwanted.
...
**Why:** Personal self-presentation copy (bio, résumé prose) is user identity, not code. Writing first and reverting after churn git history and risk bad version shipping.
...
Refused to invent a UW CS course to replace AP CS A, said plainly the gap have no substitute and ask user to name one. User then supply the real answer: Programming I, II, III, all Java.

**Why:** The honest gap prompt exactly the input needed. A fabricated course name would have shipped a fake credential and the user would never have known to correct it.
...
**Why:** User ask to remove old resume links. Deleting an export that a component render would break the site silently. The grep turn a risky delete into a verified one.
...
Both repos pass `tsc --noEmit` and `next build`, yet I told user NOT ready to commit — Education layout, skill header size, deleted HS section, 26 byte-modified images, swapped resume PDFs are all visual.
...
**How to apply:** After visual changes, state plainly that build-pass is not visual-pass. Offer `next dev`. Don't call work "ready to commit" on typecheck + build alone when the change is something a user would see.
...
**Why:** Confirm user treat `/doctor`-style audits as starting point for tune automation, not just cleanup pass — "unused" not always mean "remove," sometimes mean "make it fire under right trigger instead."
